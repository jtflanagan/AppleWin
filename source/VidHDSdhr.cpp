...
void VidHDSdhr::ImageAsset::ExtractTile(VidHDSdhr* owner, uint16_t* tile_p, uint16_t tile_xdim, uint16_t tile_ydim, uint64_t xsource, uint64_t ysource) {
...
bool VidHDSdhr::ProcessCommands() {
	...
	while (begin < end) {
		...
		case SDHR_CMD_DEFINE_IMAGE_ASSET_FILENAME: {
			...
			r->AssignByFilename(this, filename.c_str());
			if (error_flag) {
				return false;
			}
		} break;
